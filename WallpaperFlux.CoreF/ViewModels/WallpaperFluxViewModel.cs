...
namespace WallpaperFlux.Core.ViewModels
{
    public class WallpaperFluxViewModel : MvxViewModel
    {
        ...
        {
            ImageFolderUtil.LinkThemeImageFolders(ImageFolders);
        ...
        async void InitializeDisplaySettings() // waits for the DisplayCount to be set before initializing the display settings
        {
            await Task.Run(() =>
            {
                ...
                {
                    initSettings.Add(new DisplayModel(new DispatcherTimer(), NextWallpaper)
                    {
                        DisplayInterval = 0,
                        DisplayIndex = i,
        ...
        /// Image Folders
        private MvxObservableCollection<ImageFolderModel> _imageFolders = new MvxObservableCollection<ImageFolderModel>();

        public MvxObservableCollection<ImageFolderModel> ImageFolders
        {
            get => _imageFolders;
        ...
        private ImageFolderModel _selectedImageFolder;

        public ImageFolderModel SelectedImageFolder
        {
        ...
        public void AddFolder()
        {
            using (CommonOpenFileDialog dialog = new CommonOpenFileDialog())
            {
                ...
                {
                    //xImages = new DirectoryInfo(dialog.FileName).GetFiles();

                    if (!ImageFolders.ContainsImageFolder(dialog.FileName))
                    {
                        ImageFolders.Add(new ImageFolderModel {Path = dialog.FileName, Active = true});
        ...
        public void SelectImages()
        {
            ...
            MessageBoxResult result = MessageBox.Show(messageBox);
        ...
        private readonly int IMAGES_PER_PAGE = 18;
        public void RebuildImageSelector(string[] selectedImages, bool reverseOrder)
        {
            ...
            int tabCount = Math.Max(selectedImages.Length / IMAGES_PER_PAGE, 1);
            ...
            for (int i = 0; i < tabCount; i++)
            {
                ...
                for (int j = 0; j < IMAGES_PER_PAGE; j++)
                {
                    ...
                    {
                        tabModel.Images.Add(new ImageModel {Path = selectedImages[j + imageIndex]});
                    }
                    else
                    {
                        break;
